...
Pers.persist = function(obj, cb) {
	...
	self.sql3.run('insert or replace into Persistance (name, type, json) values (?,?,?)' , [obj._persistname, obj._persisttype, JSON.stringify(obj)], function (err) {
		...
		insertIndex(Object.keys(obj._persistschema).filter(function (x) {return obj._persistschema[x].searchable;}));
...
Pers.forget = function(obj, cb) {
	...
	self.sql3.run('delete from Persistance where type = ? and name = ?', [obj._persisttype, obj._persistname], function (err) {
		if (err) {
			return cb(err, undefined); 
		}
	
		for (k in obj._persistschema) {
			self.sql3.run('delete from Persistance_' + obj._persistname + ' where name = ? and field = ? and value = ?', [obj._persistname, k, obj[k]], function () {});
		}
