...
var Persistance = Object.create(new events.EventEmitter());
...
function slowStartingWith(keyPrefix, cb) {
	var self = this, k;
	...
	self.get(undefined, function (err, data) {
		...
		for (k in data) {
			if (k.slice(0, keyPrefix.length) !== keyPrefix) {
				delete data[k];
			}
		}
...
Persistance.define = function(type, schema) {
	...
	if(is_function(self.store.search)) {
		baseObj.search = function (conditions, cb) {
			...
			if(!is_object(conditions)) {
				return cb(new Error('bad conditions object'), undefined);
			}
	...
	if(is_function(self.store.startingWith)) {
		baseObj.startingWith = function (keyPrefix, cb) {
			self.store.startingWith(this, keyPrefix, cb);
		}
	} else {
		baseObj.startingWith = slowStartingWith.bind(baseObj);
	}
