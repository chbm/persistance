...
Persistance.prototype.define = function(type, schema) {
	...
	Object.defineProperty(baseObj, '_persisttype', {value: type, enumerable: true, writable: false});
	baseObj._persistschema._persisttype = baseObj._persisttype;
	...
	baseObj.create = function (name, values, cb) {
		...
		for (var k in values) {
			if (validateValue(k, obj._persistschema, values)) {
				obj[k] = values[k];
			} else {
				console.log('validate fail ', k, values[k]);
			}
		}
